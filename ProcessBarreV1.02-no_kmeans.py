...
VX.XX, 2017-07-25: This temporary version skips kmeans and just plots the mean of each row without trying to find clusters
...
class window(Frame):
    ...
    def createButton(self):
        ...
        self.quit_button=ttk.Button(self, text='Close all windows')
    ...
    def downsample(self):
        ...
        img = cv2.resize(cv2.imread(img_name), (0,0), fx=0.2, fy=0.2)
    ...
    def detect(self):
        ...
        RowMean = np.tile(labels,[rows,1])   #the tile function is what makes the bars go all the way across by repeating "labels" numcol times
        #RowMean = RowMean.T
