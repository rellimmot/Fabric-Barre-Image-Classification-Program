...
Changes since V1:
File extension no longer case sensitive. --R. Walker Shurlds
"""
from Tkinter import *
import ttk
import cv2
...
class window(Frame):
    ...
    def createButton(self):
        ...
        self.equalizer=ttk.Button(self,text='equlize image')
    ...
    def detect(self):
        ...
        rows, cols = raw.shape
        mean = np.mean(raw, axis = 1)      #mean is a 1D type (row vector)  
        mean = np.float32(mean)
        print raw.shape
        x = np.array(range(len(mean)))
        y = mean
        z = np.polyfit(x,y,2)
        mean = [v-(z[0]*ind**2+z[1]*ind+z[2]) for ind,v in enumerate(y)]
        mean = np.float32(mean)
    ...
    def equalize(self):
        ...
        img_name = img_name[:-4]+'_equlized'+'.jpg'
        cv2.imshow('%s' %img_name,img)
        cv2.imwrite(img_name,img)
